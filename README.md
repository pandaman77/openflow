...
- **Установщик** `OpenFlow_x64-setup.exe` — обычная установка, данные в `%APPDATA%\OpenFlow`.
- **Портативная версия** `OpenFlow-Portable-*-x64.zip` — распаковал и запустил `OpenFlow.exe`, без установки. Все данные (настройки, логи, модели) живут в папке `data` рядом с программой — можно носить на флешке. Маркер режима — файл `portable.txt` рядом с exe.
