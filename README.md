...
При первом запуске короткий мастер поможет выбрать микрофон и скачает модель распознавания (один раз). Модель весит гигабайты, поэтому загрузка идёт с прогрессом: видно, сколько уже скачано. Дальше всё работает без интернета.

## Обновления

Установленная версия сама проверяет при запуске, вышла ли новая. Если вышла, показывает кнопку «Обновить»: приложение скачает её, поставит и перезапустится. Обновления подписаны, посторонняя сборка не пройдёт проверку.

Портативная версия про обновление сообщит, но ставить не станет: установщик положил бы вторую копию мимо вашей папки. Для неё скачайте новый архив и распакуйте поверх.

## Сколько весят модели

Модели скачиваются один раз при первом использовании и дальше работают офлайн. Место на диске стоит прикинуть заранее:

| Модель | Для чего | Размер |
|--------|----------|--------|
| Parakeet (по умолчанию) | распознавание, русский и английский | ~2,4 ГБ |
| GigaAM | распознавание, только русский, самый быстрый на процессоре | ~850 МБ |
| Whisper base / small / large-v3-turbo | распознавание, много языков, умеет перевод | ~140 МБ / ~460 МБ / ~1,6 ГБ |
| Qwen 2.5 1.5B (опция) | умная очистка текста | ~1,1 ГБ |

В обычной установке модели лежат в кэше пользователя, в портативной версии в папке `data` рядом с программой.
